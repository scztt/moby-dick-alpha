...
def cli_main(argv=sys.argv):
    ...
    with open(keypath, 'rb') as f:
        keyder = f.read()
    rsa = RSA(keyder)
    if os.path.splitext(inpath)[1] in [".zip", ".epub"]:
        with closing(ZipFile(open(inpath, 'r'))) as inf:
            namelist = set(inf.namelist())
            for name in namelist: 
                if name[0:3] == "META": print name
        ...
        join = os.path.join
        if not(os.path.exists(join(inpath, 'META-INF/rights.xml'))) or \
           not(os.path.exists(join(inpath, 'META-INF/encryption.xml'))):
            raise ADEPTError('%s: not an ADEPT EPUB' % (inpath,))
        rights = etree.fromstring(open(join(inpath, 'META-INF/rights.xml'),"r").read())
        adept = lambda tag: '{%s}%s' % (NSMAP['adept'], tag)
        expr = './/%s' % (adept('encryptedKey'),)
        bookkey = ''.join(rights.findtext(expr))
        bookkey = rsa.decrypt(bookkey.decode('base64'))
        ...
        if bookkey[-17] != '\x00':
            raise ADEPTError('problem decrypting session key')
        encryption = open(join(inpath, 'META-INF/encryption.xml'),"r").read()
        decryptor = Decryptor(bookkey[-16:], encryption)
        kwds = dict(compression=ZIP_DEFLATED, allowZip64=False)
        with closing(ZipFile(open(outpath, 'wb'), 'w', **kwds)) as outf:
            zi = ZipInfo('mimetype', compress_type=ZIP_STORED)
            outf.writestr(zi, open(join(inpath, 'mimetype')).read())

            all_files = []
            for root, dirs, files in os.walk(join(inpath, "OEBPS")):
            ...
            for path in all_files:
                ...
                try:
                    ...
                    outf.writestr(os.path.relpath(path, inpath), decryptor.decrypt(path, data))
                except Exception, e:
                    print str(e)
